  ...
  **`TCN` layer** aggregates stacked residual blocks with additive skip connections.
  Additionally, when **Temporal Attention** (`USE_ATTENTION_MECHANISM`) is enabled:
  1. The TCN returns the full sequence (`return_sequences=True`).
  2. A `Dense(1)` layer scores each timestep, normalized via `Softmax` into attention weights.
  3. The sequences are multiplied by these weights and reduced via `tf.reduce_sum(axis=1)`.

  | Parameter | Value |
  |-----------|-------|
  | `nb_filters` | 32 (previously 128) |
  | `kernel_size` | 7 |
  | `dilations` | [1, 2, 4, 8, 16, 32, 64] |
  | `nb_stacks` | 1 |
  | `padding` | `'causal'` |
  | `use_skip_connections` | True |
  | `return_sequences` | `True` (if Attention is used) else `False` |
  | `dropout_rate` | 0.2 |
  | `activation` | `'relu'` |
  | `use_batch_norm` | True |
  ...
  **Receptive field**:
  ...
  Applied to the attention-reduced or last-timestep output vector: `(batch, 32) → (batch, 128) → (batch, 64)`.
  ...
  Input  (B, 150, 24)
    ...
    ▼
  TCN Encoder
    │  32 filters, 1 stack × 7 dilations
    │  causal padding, skip connections, BatchNorm
    │  return_sequences=True (for Attention)
    │  output: (B, 150, 32)
    ▼
  Temporal Attention (Dense(1) -> Softmax -> Multiply -> Sum)
    │  output: (B, 32)
    ▼
  Dense(128, gelu) → (B, 128)
    │
    ▼
  ...
  | Component | Value |
  |-----------|-------|
  | **Optimizer** | `Adam` or `LossScaleOptimizer` (mixed precision) |
  | **Loss** | Standard `SparseCategoricalCrossentropy` or `label_smoothing_sparse_categorical_crossentropy` |
  | **Metrics** | `sparse_categorical_accuracy`, `SparseTopKCategoricalAccuracy(k=3)`, `SparseTopKCategoricalAccuracy(k=5)` |

  ---

  ## 5. Loss Functions

  ### 5.1 `SparseCategoricalCrossentropy` / Label Smoothing — Active

  The standard active loss is pure `SparseCategoricalCrossentropy`. 
  Alternatively, `label_smoothing_sparse_categorical_crossentropy(smoothing=0.1)` is introduced to mitigate model overconfidence. It redistributes probability mass (e.g., smoothing hard targets to prevent over-certainty), serving as a regularized alternative.

  ### 5.2 `spatial_loss(y_true, y_pred, alpha=0.1)` — Available, Not Active
  ...
  ### 5.3 `SpatialCrossEntropy(alpha=2.0)` — Available, Not Active
  ...
  | `EarlyStopping` | Monitor: `val_loss` (or similar), restores best weights at the end |
  | `CosineAnnealingWarmupScheduler`| Linear learning rate warmup followed by a cosine decay (replaces `ReduceLROnPlateau`) |
  | `ModelCheckpoint` | Saves best model to `.keras` files |
  | `ConfusionMatrixCallback` | Appends a hook to track and output confusion matrix at epoch ends |
  | `CSVLogger` | Stores epoch metrics to a `.csv` log for tracking |
  ...
  The notebook contains several alternative model definitions that are available:

  | Model | Description | Backend |
  |-------|-------------|---------|
  | `MDN_TCN` (Mixture Density Network) | TCN Encoder coupled with an MDN head that outputs Gaussian Mixture Model parameters (pi, mu, sigma) for probabilistic spatial regression. Extends the primary TCN capability to multi-modal target landscapes. | Keras |
